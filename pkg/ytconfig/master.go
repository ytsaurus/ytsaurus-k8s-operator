...
func GetMasterCellDescriptors(primary *ytv1.MastersSpec, secondary []ytv1.MastersSpec) map[string]MasterCellDescriptor {
	...
	// NOTE: Not filling default roles for primary master cell. Use default from master itself.
	if primary.Roles != nil {
		descriptors[fmt.Sprintf("%d", primary.CellTag)] = MasterCellDescriptor{
			Roles: ptr.To(*primary.Roles),
		}
	}
	for _, cell := range secondary {
		descriptors[fmt.Sprintf("%d", cell.CellTag)] = MasterCellDescriptor{
			Roles: ptr.To(ytv1.GetMasterCellRoles(cell.Roles, false, true)),
		}
	}
	return descriptors
}
