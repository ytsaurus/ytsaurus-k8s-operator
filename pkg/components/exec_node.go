...
type execNode struct {
	...
	master       Component
	execNodeSpec *ytv1.ExecNodesSpec
}
...
func (n *execNode) doBuild() error {
	...
	setContainerPrivileged := func(ct *corev1.Container) {
		...
		ct.SecurityContext.Privileged = ptr.Bool(n.execNodeSpec.Privileged)
	}

	if len(podSpec.Containers) != 1 {
		log.Panicf("length of exec node containers is expected to be 1, actual %v", len(podSpec.Containers))
	}
	...
	for _, sidecarConfig := range n.execNodeSpec.Sidecars {
		sidecarSpec := corev1.Container{}
		if err := yaml.Unmarshal([]byte(sidecarConfig), &sidecarSpec); err != nil {
			return err
		}
		podSpec.Containers = append(podSpec.Containers, sidecarSpec)
	}
