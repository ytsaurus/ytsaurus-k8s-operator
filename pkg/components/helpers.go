...
const (
	...
	OnDeleteUpdateModeName      = "OnDelete"
...
func handleBulkUpdatingClusterState(
	...
	if ytsaurus.ShouldRunPreChecks(cmp.GetType(), cmp.GetShortName()) {
		if status, err := runPrechecks(ctx, ytsaurus, cmp, BulkUpdateModeName); status != nil {
			return status, err
		}
	}
...
func handleOnDeleteUpdatingClusterState(
	...
	// Run pre-checks if needed
	if ytsaurus.ShouldRunPreChecks(cmp.GetType(), cmp.GetShortName()) {
		if status, err := runPrechecks(ctx, ytsaurus, cmp, OnDeleteUpdateModeName); status != nil {
			return status, err
		}
	}
	...
	return ptr.To(ComponentStatusWaitingFor("manual pod update by user")), err
}

func runPrechecks(ctx context.Context, ytsaurus *apiproxy.Ytsaurus, cmp Component, updateMode string) (*ComponentStatus, error) {
	ytsaurus.SetUpdateStatusCondition(ctx, metav1.Condition{
		Type:    fmt.Sprintf("%s%s%s", cmp.GetShortName(), updateMode, consts.ConditionPreChecksRunning),
		Status:  metav1.ConditionTrue,
	...
	if preCheckStatus.SyncStatus != SyncStatusReady {
		...
		ytsaurus.SetUpdateStatusCondition(ctx, metav1.Condition{
			Type:    fmt.Sprintf("%s%s%s", cmp.GetShortName(), updateMode, string(SyncStatusBlocked)),
			Status:  metav1.ConditionTrue,
	...
	ytsaurus.SetUpdateStatusCondition(ctx, metav1.Condition{
		Type:    fmt.Sprintf("%s%s%s", cmp.GetShortName(), updateMode, consts.ConditionPreChecksCompleted),
		Status:  metav1.ConditionTrue,
