	...
	srv := newServer(
		...
		[]ConfigGenerator{
			ServerConfigGenerator(l, func() ([]byte, error) {
				return cfgen.GetCypressProxiesConfig(resource.Spec.CypressProxies)
			}),
		},
...
func (cyp *CypressProxy) Sync(ctx context.Context, dry bool) (ComponentStatus, error) {
	...
	if cyp.ytsaurus.GetClusterState() == ytv1.ClusterStateUpdating {
		if status, err := dispatchComponentUpdate(ctx, cyp.ytsaurus, cyp, &cyp.component, cyp.server, dry); status != nil {
			return *status, err
		}
	}
