...
func TestMasterFlow(t *testing.T) {
	...
	// initial creation
	master := NewMaster(cfgen, ytsaurus, &fakeYtsaurusForMaster{})
	testutil.Eventually(h, "ms became ready", func() bool {
		st, err := master.Status(ctx)
		require.NoError(t, err)
		if st.SyncStatus == SyncStatusReady {
			return true
		}
		require.NoError(t, master.Sync(ctx))
		return false
	})

	cmData := testutil.FetchConfigMapData(h, "yt-master-config", "ytserver-master.yson")
