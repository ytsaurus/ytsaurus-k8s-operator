...
var (
	...
	YtsaurusPrevEpoch = os.Getenv("YTSAURUS_PREV_EPOCH")
	YtsaurusTestEpoch = os.Getenv("YTSAURUS_TEST_EPOCH")
	YtsaurusNextEpoch = os.Getenv("YTSAURUS_NEXT_EPOCH")
...
func init() {
	...
	if YtsaurusTestEpoch == "" {
		YtsaurusTestEpoch = "CURRENT"
	}

	for _, name := range names {
		images[name] = map[string]ImageEntry{}
		for _, epoch := range epochs {
			ver := GetenvOr(name+"_VERSION_"+epoch, epoch)
			image := ""
			if ver != epoch {
				tag := ver
				if name == "YTSAURUS" {
				...
				image = GetenvOr(name+"_IMAGE", YtsaurusRegistry+"/"+strings.ToLower(name)) + ":" + tag
			}
	...
	if _, ok := images["YTSAURUS"][YtsaurusTestEpoch]; !ok {
		images["YTSAURUS"][YtsaurusTestEpoch] = ImageEntry{
			Epoch:   "CURRENT",
			Version: YtsaurusTestEpoch,
			Image:   GetenvOr("YTSAURUS_IMAGE", YtsaurusRegistry+"/ytsaurus") + ":stable-" + YtsaurusTestEpoch,
		}
	}

	tlsReady := os.Getenv("YTSAURUS_TLS_READY") != ""
	for key, yt := range images["YTSAURUS"] {
		choose := func(name string) string {
			if e, ok := images[name][yt.Epoch]; ok {
				return e.Image
			}
			return images[name][YtsaurusTestEpoch].Image
		}
		Images[key] = YtsaurusImages{
			YtsaurusVersion: yt.Version,
			...
			MutualTLSReady: tlsReady || yt.Version >= "25.3",
		}
	}

	if index := slices.Index(epochs, images["YTSAURUS"][YtsaurusTestEpoch].Epoch); index != -1 {
		if YtsaurusPrevEpoch == "" && index > 0 {
			YtsaurusPrevEpoch = epochs[index-1]
		}
		if YtsaurusNextEpoch == "" && index < len(epochs)-1 {
			YtsaurusNextEpoch = epochs[index+1]
		}
	}
	PrevImages = Images[YtsaurusPrevEpoch]
	TestImages = Images[YtsaurusTestEpoch]
	NextImages = Images[YtsaurusNextEpoch]
}
