...
// What do we want to test with integration:
//
//	[x] on deploy remote nodes operator creates
//		- sts with expected spec
//		- configmaps with expected content
//	[x] on update of remote ytsaurus operator recreates configmap
//	[?] on update of remote node spec image operator recreates sts?
//	[x] on update of remote node spec instance count operator adds pods
//	[] on update of remote node spec params operator recreates smth
...
func TestRemoteExecNodesCnangeInstanceCount(t *testing.T) {
...
func updateObject(h *testHelper, emptyObject, newObject client.Object) {
	...
	err := k8sCli.Get(context.Background(), key, emptyObject)
	require.NoError(h.t, err)
