...
func (r *YtsaurusReconciler) Sync(ctx context.Context, resource *ytv1.Ytsaurus) (ctrl.Result, error) {
	...
	// TODO: move that after statuses collection and before sync, so in case
	// correct stage is also indicated.
	updateState := stageToUpdateStatus(st)
	err = stateManager.SetClusterUpdateState(ctx, updateState)
	if err != nil {
		return ctrl.Result{}, fmt.Errorf("failed to save update state to %s: %w", updateState, err)
	}
	return requeue, nil
}
