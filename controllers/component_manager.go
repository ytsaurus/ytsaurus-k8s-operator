...
func NewComponentManager(
	...
	nodeCfgGen := ytconfig.NewLocalNodeGenerator(resource, clusterDomain)

	allComponents, s, qt := getComponentInstances(cfgen, nodeCfgGen, ytsaurus)
...
func getComponentInstances(
	...
	nodeCfgGen *ytconfig.NodeGenerator,
	ytsaurus *apiProxy.Ytsaurus,
) ([]components.Component, components.Component, components.Component) {
	var (
		d = components.NewDiscovery(cfgen, ytsaurus)
		m = components.NewMaster(cfgen, ytsaurus)

		hps []components.Component
	)
	...
	if resource.Spec.QueryTrackers != nil && resource.Spec.Schedulers != nil && resource.Spec.TabletNodes != nil && len(resource.Spec.TabletNodes) > 0 {
		qt = components.NewQueryTracker(cfgen, ytsaurus, yc, tnds)
		allComponents = append(allComponents, qt)
	}
