...
var _ = Describe("Basic e2e test for Ytsaurus controller", Label("e2e"), func() {
	...
	JustBeforeEach(func(ctx context.Context) {
		...
		if ytBuilder.WithHTTPSProxy {
			...
			Expect(rootCAs.AppendCertsFromPEM(caBundleCertificates)).To(BeTrueBecause("CA bundle certificates should be parseable"))
	...
	Context("Update scenarios", Label("update"), func() {
		...
		Context("With config overrides", Label("overrides"), func() {
			...
			It("Applies cypress patch by config overrides", func(ctx context.Context) {
				...
				Eventually(func() (any, error) {
					...
					return value, err
				}, upgradeTimeout, pollInterval).Should(BeTrueBecause("scheduler config issue_temporary_token should be true"))
			})
		...
		Context("With tablet nodes", Label("tablet"), func() {
			...
			It("Should run and try to update Ytsaurus with tablet cell bundle which is not in `good` health", func(ctx context.Context) {
				...
				Eventually(func() bool {
					...
					return len(notGoodBundles) > 0
				}, reactionTimeout, pollInterval).Should(BeTrueBecause("some tablet cell bundles should not be in good health after banning nodes"))
		...
		Context("With data nodes", Label("data"), func() {
			...
			It("Should run and try to update Ytsaurus with lvc", func(ctx context.Context) {
				...
				Eventually(func() bool {
					...
					return lvcCount > 0
				}, reactionTimeout, pollInterval).Should(BeTrueBecause("lost vital chunks count should be greater than zero"))
		...
		Context("With queue agent", Label("queue-agent"), func() {
			...
			It("Should run with queue agent", func(ctx context.Context) {
				Expect(ytClient.NodeExists(ctx, ypath.Path("//sys/queue_agents/queues"), nil)).Should(BeTrueBecause("//sys/queue_agents/queues node should exist"))
				Expect(ytClient.NodeExists(ctx, ypath.Path("//sys/queue_agents/consumers"), nil)).Should(BeTrueBecause("//sys/queue_agents/consumers node should exist"))
			})
	...
	Context("Remote ytsaurus tests", Label("remote"), func() {
		...
		Context("With remote tablet nodes", Label("tablet"), func() {
			...
			It("Should create ytsaurus with remote tablet nodes and write to a table", func(ctx context.Context) {
				...
				}, upgradeTimeout, pollInterval).Should(BeTrueBecause("dynamic table should be mounted"))
...
exec "$@"`
		...
		Context("With RPC proxy", Label("rpc-proxy"), func() {
			...
			It("Rpc proxies should require authentication", func(ctx context.Context) {
				...
				Expect(yterrors.ContainsErrorCode(err, yterrors.CodeRPCAuthenticationError)).Should(BeTrueBecause("unauthenticated RPC request should fail with authentication error"))
		...
		Context("With host network", Label("host-network"), func() {
			...
			It("Sensors should be annotated with host", func(ctx context.Context) {
				...
				Expect(ok).Should(BeTrueBecause("response body should contain 'sensors' array"))
				Expect(sensors).ShouldNot(BeEmpty())

				sensor, ok := sensors[0].(map[string]any)
				Expect(ok).Should(BeTrueBecause("sensor entry should be a map"))
				Expect("labels").Should(BeKeyOf(sensor))

				labels, ok := sensor["labels"].(map[string]any)
				Expect(ok).Should(BeTrueBecause("sensor 'labels' should be a map"))
		...
		DescribeTableSubtree("With Bus RPC TLS", Label("tls"), func(epoch string) {
			...
			It("Verify that mTLS is active", Label(epoch), Label(images.YtsaurusVersion.String()), func(ctx context.Context) {
				...
				By("Waiting http proxy", func() {
					...
					}, bootstrapTimeout, pollInterval, ctx).MustPassRepeatedly(5).Should(BeTrueBecause("root node should exist after certificate reissue"))
				})
			...
			func(componentType consts.ComponentType, stsName string) {
				...
				It("Should update "+stsName+" in bulkUpdate mode and have Running state", func(ctx context.Context) {
					...
					Eventually(func() bool {
						...
						return true
					}, upgradeTimeout, pollInterval).Should(BeTrueBecause("all %s pods should move to the new revision", stsName))
			...
			func(componentType consts.ComponentType, stsName string) {
				...
				It("should update "+stsName+" with OnDelete strategy and have cluster Running state", func(ctx context.Context) {
					...
					EventuallyObject(ctx, &sts, reactionTimeout).Should(WithTransform(
						...
						BeTrueBecause("StatefulSet should have differing current and update revisions"),
					))
					...
					Consistently(func() bool {
						...
						return true
					}, 10*time.Second).Should(BeTrueBecause("all %s pods should stay on old revision", stsName))
			...
			func(componentType consts.ComponentType, stsName string, minReady int32) {
				...
				It("should update "+stsName+" with RollingUpdate strategy and have cluster Running state", func(ctx context.Context) {
					...
					EventuallyObject(ctx, &sts, reactionTimeout).Should(WithTransform(
						...
						BeTrueBecause("StatefulSet should have RollingUpdate strategy with correct maxUnavailable"),
					))
		...
		Context("concurrent rolling update for multiple data-node groups", Label("rollingupdate-dnd"), func() {
			...
			It("should update data-node groups sequentially (default first, then 3c25) and reach Running state", func(ctx context.Context) {
				...
				Eventually(func() bool {
					...
					return true
				}, upgradeTimeout, pollInterval).Should(BeTrueBecause("all default rack pods should move to the new revision"))
				...
				Eventually(func() bool {
					...
					return true
				}, upgradeTimeout, pollInterval).Should(BeTrueBecause("all named rack pods should move to the new revision"))
...
func checkClusterHealth(ctx context.Context, ytClient yt.Client) {
	...
	By("Checking cluster alerts", func() {
		...
		}, reactionTimeout, pollInterval).Should(BeTrueBecause("cluster should have no alerts or errors"))
	})
...
func checkChunkLocations(ytClient yt.Client) {
	...
	if err == nil {
		Expect(realChunkLocationsValue).Should(BeTrueBecause("enable_real_chunk_locations should be true"))
	}
	...
	for _, node := range values {
		Expect(node.Attrs["use_imaginary_chunk_locations"]).ShouldNot(BeTrueBecause("use_imaginary_chunk_locations should not be set on data nodes"))
	}
}
...
func (o *TestOperation) Wait() *yt.OperationStatus {
	...
	Eventually(func(ctx context.Context) bool {
		...
		return false
	}, operationTimeout, operationPollInterval, specCtx).Should(BeTrueBecause("operation should complete successfully"))
	return opStatus
}
